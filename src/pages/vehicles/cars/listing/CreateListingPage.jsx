// client/src/pages/vehicles/cars/listing/CreateListingPage.jsx
import React, { useState } from 'react';
import { useNavigate, Link } from 'react-router-dom';

export function CreateCarListing({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Payload: Supabase-UUID (profile_id) + Formulardaten für Autos
    const payload = {
      profile_id: user?.id, 
      ...formData
    };
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Fehler beim Speichern des Auto-Inserats.');
      }

      const result = await response.json();
      console.log('Auto erfolgreich gespeichert:', result);
  ...
  return (
    <div className="card" style={{ margin: '0 auto', textAlign: 'left', maxWidth: '600px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <Link to="/vehicles/cars/listing" style={{ textDecoration: 'none' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div>
          ...
          <input 
            ...
            placeholder="z.B. BMW 320d M-Paket"
          />
        </div>
        ...
        <div>
          ...
          <textarea 
            ...
            rows="4"
            style={{ width: '100%', padding: '8px', boxSizing: 'border-box' }}
            placeholder="Kilometerstand, Zustand, Ausstattung etc."
          />
        </div>
        ...
        >
          {loading ? 'Wird gespeichert...' : 'Auto-Inserat veröffentlichen'}
        </button>
      </form>
    </div>
  );
